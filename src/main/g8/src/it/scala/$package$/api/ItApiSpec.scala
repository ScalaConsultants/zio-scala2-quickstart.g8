...
object ItApiSpec extends ZioRouteTest with PosgresContainer {
  ...
  private val specs: Spec[ItemRepository with Blocking with Api with Clock, TestFailure[Throwable], TestSuccess] =
    suite("Api")(
      testM("Add item on POST to '/items'") {
        ...
        for {
          ...
          resultCheck <- effectBlocking(request ~> routes ~> check {
                          ...
                          assert(theBody)(equalTo(Item(ItemId(1), "name", 100.0)))
                        })
      ...
      testM("Return all items on GET to '/items'") {
        val items = List(Item(ItemId(1), "name", 100.0), Item(ItemId(2), "name2", 200.0))
      ...
      testM("Delete item on DELETE to '/items/:id'") {
        ...
        for {
          ...
          contentsCheck <- assertM(allItems)(hasSameElements(items.tail))
        } yield resultCheck && contentsCheck
  ...
  def spec = specs.provideLayer(env.mapError(TestFailure.die))
}
